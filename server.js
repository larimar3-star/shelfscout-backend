...
app.get('/lookup', async (req, res) => {
  ...
  try {
    ...
    const { asin, title, brand, category, imageUrl, price, reviews, rating, detailUrl } = searchResult;
    const keepaData = KEEPA_KEY ? await keepaLookup(asin) : null;
    const intel = buildIntel({ asin, barcode, title, brand, category, imageUrl, price,
                               details: { reviews, rating, detailUrl }, keepaData });

    console.log(`[lookup] ✅ ${title} | ASIN=${asin} | score=${intel.s.ov}`);
    res.json(intel);
...
app.get('/debug/barcode', async (req, res) => {
  ...
  const found = Object.entries(results).filter(([,v])=>v.title||(v.items>0));
...
// Strategy: try UPC → EAN → both ID types → keyword search
async function papiSearchByBarcode(barcode) {
  ...
  // ── Attempt 1: GetItems with IdType=UPC (12-digit) or EAN (13-digit) ──
  const idType = barcode.length === 13 ? 'EAN' : 'UPC';
  const altType = barcode.length === 13 ? 'UPC' : 'EAN'; // try opposite too
  ...
  try {
    ...
    const response = await papiRequest('GetItems', {
      ItemIds:     [barcode],
      ItemIdType:  idType,
      SearchIndex: 'All',
      PartnerTag:  PA_PARTNER_TAG,
  ...
  // Try alternate ID type (UPC 12-digit sometimes registered as EAN with leading 0)
  const altBarcode = barcode.length === 12 ? '0' + barcode : barcode.slice(1);
  try {
    console.log(`[paapi] Attempt 2: GetItems IdType=${altType} barcode=${altBarcode}`);
    const response = await papiRequest('GetItems', {
      ItemIds:     [altBarcode],
      ItemIdType:  altType,
      SearchIndex: 'All',
  ...
  // ── Attempt 3: SearchItems with barcode as keyword ──────────
  // This finds products where the barcode appears in title/description
  try {
    console.log(`[paapi] Attempt 3: SearchItems keyword=${barcode}`);
    const response = await papiRequest('SearchItems', {
      Keywords:    barcode,
      SearchIndex: 'All',
      PartnerTag:  PA_PARTNER_TAG,
      PartnerType: 'Associates',
    ...
    if (items && items.length > 0) {
      // Filter: first item whose ExternalIds contains our barcode (best match)
      const exact = items.find(item => {
        ...
        return upcs.includes(barcode) || eans.includes(barcode) ||
               upcs.includes(altBarcode) || eans.includes(altBarcode);
      });
      const best = exact || items[0]; // fall back to first result if no exact match
      console.log(`[paapi] ✅ Found via keyword${exact ? ' (exact UPC match)' : ' (first result)'}: ${best.ItemInfo?.Title?.DisplayValue}`);
      return extractItemData(best);
    }
    console.log(`[paapi] No keyword results either`);
  } catch (e) {
    console.warn(`[paapi] SearchItems keyword error:`, e.response?.data?.Errors?.[0]?.Message || e.message);
  }
